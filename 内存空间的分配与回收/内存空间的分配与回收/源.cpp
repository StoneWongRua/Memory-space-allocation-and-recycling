...
struct Allocation_Table {
	int initial_address;//起始地址
	int length;//长度
	int allocation_table_status = 0;//设置为空，表示未被分配(进程号)
...
public:
	MemorySpace(int n);
	~MemorySpace();
	void ApplicateSpace();//内存申请 进程名，进程号，分区大小
	void AddTableNode(int initial_address,int length, int allocation_table_status);//登记已分配区表
	void RemoveTableNode(int allocation_table_status);//删除已分配空闲区
	void FreeSpace();//内存回收
	void MergeFreeSpace(int number);//合并分区 参数：进程号
	void PrintLink(bool flag);//打印空闲区所有结点（包括已分配的空闲区）
	void PrintTable();//打印已分配表
};
...
MemorySpace::MemorySpace(int n)//参数主存的容量
{
	...
	Free_Node *firstnode = new Free_Node("freespace",0,n);//链表的第一个结点，未分配，空间为n
...
void MemorySpace::ApplicateSpace()
{
	...
	if (size <= head_free->size)//判断内存空间剩余所有容量是否大于等于申请需求(包括碎片)
	{
		//cout << p_new->process_name << "  " << p_new->number << " " << p_new->size;
		Free_Node *p_work = head_free->next;
		while (p_work)
		{
			if (p_work->status == true && p_work->size > size)//如果等于空闲 && 空间 > 需求量
			{
				...
				p_work->size = p_work->size - p_new->size;//p_work属于空间区，p_new分配的主存空间是从p_work上分割出来的

				p_new->address = p_work->address;//起始地址的设置
				p_work->address = p_work->address + size;
				AddTableNode(p_new->address,size,p_new->number);//登记已分配区说明表
				flag = true;//分配标志设置为成功
				break;
			}
			else if (p_work->status == true && p_work->size == size)//如果等于空闲 && 空间 = 需求量
			{
				...
				AddTableNode(p_work->address, size, p_work->number);//登记已分配区说明表
				flag = true;//分配标志设置为成功
				break;
			}
			p_work = p_work->next;
...
void MemorySpace::AddTableNode(int initial_address, int length, int allocation_table_status)//建立已分配说明表
{
	
	Allocation_Table *q_temp = new Allocation_Table();
	q_temp->allocation_table_status = allocation_table_status;//进程号
	q_temp->length = length;//分区大小
	q_temp->initial_address = initial_address;//起始地址
...
void MemorySpace::RemoveTableNode(int allocation_table_status)
{
	...
	while (p_delete)
	{
		if (p_delete->allocation_table_status == allocation_table_status)//找到需要删除的结点，删除
		{
...
void MemorySpace::FreeSpace()//释放空间
{
	...
	if (find == true)
	{
		...
		RemoveTableNode(p_work->number);//将已分配表中对应的表目删除
		//实现释放主存
		p_work->process_name = "freespace";//进程名修改为 freespace
...
void MemorySpace::MergeFreeSpace(int number)
{
	...
	else if (p_work->prev->status == false && p_work->next->status == false )//释放分区的前、后邻接分区都是已分配区，没有合并的问题存在
	{
		p_work->number = 0;//指针不需要变动，将相应结点的进程号设为0
	}
	else if (p_work->prev->status == true && p_work->next->status == false)//释放分区的前邻接分区是空闲区，后邻接分区是已分配区。
	{
	...
	else if (p_work->prev->status == false && p_work->next->status == true)//释放区的前邻接分区是已分配区，后邻接分区是空闲区。
	{
	...
	else if (p_work->prev->status == true && p_work->next->status == true)//释放区的前、后邻接分区都是空闲区。
	{
		...
		Free_Node *p_tempnext = p_work->next;
		p_tempprev->size = (p_tempprev->size + p_work->size + p_tempnext->size);
		if (p_tempnext->next == NULL)//释放区的后邻接分区是空闲区，则不需要设置前驱指针
		{
			p_tempprev->next = NULL;
		}
		else {
			p_tempnext->next->prev = p_tempprev;//释放区的后邻接分区非空闲区，则需要设置前驱指针
			p_tempprev->next = p_tempnext->next;
		}
		delete p_work;
		delete p_tempnext;
...
void MemorySpace::PrintLink(bool flag)
{
	if (flag == true)//输出完整链表
	{
		...
		while (p_temp)
		{
			cout << "进程名：" <<p_temp->process_name << " 进程号：" << p_temp->number << " 空闲分区大小：" << p_temp->size << " 分配状态：" << p_temp->status <<" 首地址："<<p_temp->address<< endl;
			p_temp = p_temp->next;
		}
	}
	else//输出未分配的分区
	{
		...
		while (p_temp)
		{
			if(p_temp->status == true)
				cout << "进程名：" <<p_temp->process_name << " 进程号：" << p_temp->number << " 分区大小：" << p_temp->size << " 分配状态：" << p_temp->status << " 首地址：" << p_temp->address << endl;
			p_temp = p_temp->next;
		}
	}
}

void MemorySpace::PrintTable()//输出已分配说明表
{
	cout << "已分配说明表：" << endl;
	Allocation_Table *p_work = head_allocation->next;
	while (p_work)
	{
		cout << "进程号：" << p_work->allocation_table_status << " 起始地址：" << p_work->initial_address << " 分区大小：" << p_work->length << endl;
		p_work = p_work->next;
	}
}
...
int main()
{
	...
	do
	{
		...
		cout << "        〓〓〓★ ☆ 4. 打印主存空闲分区  ☆ ★〓〓〓" << endl;
		cout << "        〓〓〓★ ☆ 5.打印已分配区说明表 ☆ ★〓〓〓" << endl;
...
//问题
//1.内存全部分配完，无空闲空间时，删除最后一个结点报错
//2.同样问题，（释放区的前邻接分区是已分配区，后邻接分区是空闲区。）假设最后一个结点为空闲区，删除倒数第二个结点，报错
//3.同样问题，（释放区的前、后邻接分区都是空闲区。）假设最后一个结点和倒数第三个结点为空闲区，删除倒数第二个结点，报错
//4.有一行代码一直不执行，
//5.FreeSpace()函数里（198行）删除结点的时候找不到相应的进程号 ？
//6.开始的时候空闲区链 采用的是 单链表 考虑到内存回收删除结点 后来写到一般的时候将 单链表 改为 双向链表
//解决
//双向链表删除最后一个结点操作
//写的时候有点那个，赋值写成了等于，下断点监控变量的时候if判断没问题，函数体就不执行
